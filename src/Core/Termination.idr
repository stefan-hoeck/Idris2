...
checkIfGuarded : {auto c : Ref Ctxt Defs} ->
                 FC -> Name -> Core ()
checkIfGuarded fc n
    = do log TotalityTerminationGuarded 6 $ "Check if Guarded: " ++ show !(toFullNames n)
         defs <- get Ctxt
...
mutual
  ...
  findSCcall defs env g pats fc fn_in arity args
      ...
      = do Just gdef <- lookupCtxtExact fn_in (gamma defs)
           ...
           log TotalityTerminationSizechange 10 $ "Looking under " ++ show !(toFullNames fn)
...
calculateSizeChange loc n
    = do log TotalityTerminationSizechange 5 $ "Calculating Size Change: " ++ show !(toFullNames n)
         defs <- get Ctxt
...
checkSC defs f args path
   = do log TotalityTerminationSizechange 7 $ "Checking Size Change Graph: " ++ show !(toFullNames f)
        let pos = (f, map (map Builtin.fst) args)
        if pos `elem` path
           then do log TotalityTerminationSizechangeInPath 8 $ "Checking arguments: " ++ show !(toFullNames f)
                   toFullNames $ checkDesc (mapMaybe (map Builtin.snd) args) path
           else case !(lookupCtxtExact f (gamma defs)) of
                     Nothing => do log TotalityTerminationSizechangeIsTerminating 8 $ "Size Change Graph is Terminating for: " ++ show !(toFullNames f)
                                   pure IsTerminating
                     Just def => do log TotalityTerminationSizechangeNeedsChecking 8 $ "Size Change Graph needs traversing: " ++ show !(toFullNames f)
  ...
  where
    ...
    checkCall path sc
        = do let inpath = fnCall sc `elem` map fst path
             ...
             log TotalityTerminationSizechangeCheckCall 8 $ "CheckCall Size Change Graph: " ++ show !(toFullNames (fnCall sc))
             ...
             if not inpath
                then case term of
                       NotTerminating (RecPath _) =>
                          ...
                          do log TotalityTerminationSizechangeCheckCallInPathNotRestart 9 $ "ReChecking Size Change Graph: " ++ show !(toFullNames (fnCall sc))
                             args' <- initArgs (length (fnArgs sc))
                             checkSC defs (fnCall sc) args' path
                       t => do log TotalityTerminationSizechangeCheckCallInPathNotReturn 9 $ "Have result: " ++ show !(toFullNames (fnCall sc))
                               pure t
                else do log TotalityTerminationSizechangeCheckCallInPath 9 $ "Have Result: " ++ show !(toFullNames (fnCall sc))
                        pure term
...
calcTerminating loc n
    = do defs <- get Ctxt
         log TotalityTerminationCalc 7 $ "Calculating termination: " ++ show !(toFullNames n)
...
checkTerminating loc n
    = do tot <- getTotality loc n
         log TotalityTermination 6 $ "Checking termination: " ++ show !(toFullNames n)
...
calcPositive loc n
    = do defs <- get Ctxt
         log TotalityPositivity 6 $ "Calculating positivity: " ++ show !(toFullNames n)
...
checkPositive loc n_in
    = do n <- toResolvedNames n_in
         ...
         log TotalityPositivity 6 $ "Checking positivity: " ++ show !(toFullNames n)
         case isTerminating tot of
              Unchecked =>
